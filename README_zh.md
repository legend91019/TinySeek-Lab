...
注意：本项目不是复现 DeepSeek 的最终效果，而是复现它的研究路径：

1. 先训练 Dense decoder-only base LM。
2. 复现 DeepSeek LLM 风格的 batch size / learning rate 小规模网格搜索。
3. 升级基础 block：RMSNorm、RoPE、SwiGLU、GQA。
4. 把 Dense FFN 替换成 DeepSeekMoE 风格的 routed experts。
5. 研究 MoE 的负载均衡、辅助损失、routing collapse 和专家分化。
6. 加入教学版 MLA，理解 KV cache 压缩的动机。
7. 继续做 SFT、reasoning cold start、DPO、rule-based GRPO mini。
...
```mermaid
flowchart LR
  A["阶段 0<br/>Dense LM 基线"] --> B["阶段 1<br/>LR / Batch 网格搜索"]
