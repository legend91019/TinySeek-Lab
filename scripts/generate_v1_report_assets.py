...
def merged_rows(cost_rows: list[dict], evals: dict[str, dict]) -> list[dict]:
    ...
    for cost in cost_rows:
        ...
        copy_acc = ev.get("copy", {}).get("accuracy", "")
        qa_acc = ev.get("keyword_qa", {}).get("accuracy", "")
...
def write_table(rows: list[dict], out: Path, *, zh: bool) -> None:
    ...
    for row in rows:
        lines.append(
            "| `{run}` | {steps} | {params} | {activated} | {peak:.2f} | {hours:.4f} | {cost:.4f} | {val} | {eval_loss} | {ppl} | {add} | {copy} | {qa} |".format(
                run=row["run_name"],
                steps=row["step"],
                ...
                copy=f"{to_float(row['copy_accuracy']):.2f}" if row.get("copy_accuracy") != "" else "N/A",
                qa=f"{to_float(row['keyword_qa_accuracy']):.2f}" if row.get("keyword_qa_accuracy") != "" else "N/A",
            )
        )
    lines.extend(
        [
