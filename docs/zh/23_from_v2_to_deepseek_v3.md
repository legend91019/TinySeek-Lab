...
这里 sigmoid 不跨 expert 归一化，每个 affinity 独立落在 `(0,1)`。选择时才对
$s_n$ 做 `torch.topk`。`expert_bias` 是 `register_buffer` 状态，会随设备移动，
...
`gather(1,top_indices)` 从 `[N,E]` affinity 按 `[N,K]` id 取值；真实代码中的
`clamp_min(1e-9)` 给分母加下界，防止极端除零。
...
`torch.bincount(...,minlength=E)` 得到 $c_e$，`torch.sign` 只保留更新方向；
`@torch.no_grad()` 保证原地 `add_`/`sub_` 不进入 autograd 图。
...
分号表示沿最后一维拼接，而不是相加。
最后这个 RMSNorm 对应真实代码
`self.lm_head(self.norm(previous_hidden[:, :-1]))`，不能从精确映射里省略。
...
`torch.cat(...,dim=-1)` 把两路 `[B,T-1,D]` 变成 `[B,T-1,2D]`，
`nn.Linear(2D,D)` 再投回 block 宽度。多个 depth loss 用
`torch.stack(mtp_losses).mean()` 聚合，以保留 device、dtype 和 autograd。

这个对齐很重要。多一个或少一个 slice，模型就会预测错位置，loss 仍可能下降，却学的是错误任务。

## 7. 为什么共享 Embedding 和 LM Head

Stage 3 直接复用上面代码中的 `self.embed`、`self.norm` 和 `self.lm_head`。共享可以
减少额外参数，并保证所有预测深度使用同一 token 空间。教学版 MTP 模块内部使用
一个 Dense Transformer block，重点保留顺序预测、共享 embedding/head 和 label
shift；它没有复刻 V3 的所有工程细节。
