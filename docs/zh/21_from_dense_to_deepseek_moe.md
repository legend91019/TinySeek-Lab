...
`nn.Linear(D,E,bias=False)` 对应 $W_r$，把 `[B*T,D]` 变成 `[B*T,E]`；
`F.softmax(...,dim=-1)` 只在 expert 轴归一化，因此每一行之和为 1。
...
`torch.topk` 同时返回概率值与 expert id；`.sum(...,keepdim=True)` 产生
`[B*T,1]` 分母并广播除回 `[B*T,K]`。
...
`top_indices == expert_id` 生成 `[B*T,K]` 布尔 mask；
`nonzero(as_tuple=True)` 返回 token 行号和 top-k slot；advanced indexing 只取当前
expert 的 token；`unsqueeze(-1)` 让 `[N_e]` 权重广播到 hidden 维；`+=` 把多个
专家贡献累加回原位置。
...
当配置允许的 $E_s=0$ 时，代码保留 `shared_out=0`，公式退化为
$y_n=y_n^{routed}$，不会执行除零。
...
`F.one_hot(...,num_classes=E)` 构造指示函数，`.float().mean(dim=0)` 得到 $A_e$，
`torch.sum` 对 expert 轴求和。这个 proxy 可读、可微到 router probability，但
不是 DeepSeek 所有版本负载损失的逐项复刻。

上式精确对应 `model/stages/stage1_deepseek_moe.py` 的教学模型。正式消融使用的
`model/tinyseek.py` 调用 `sequence_load_balance_loss`，按 sequence 并使用全部
top-k assignment：
...
代码把 `top_indices [B*T,K]` 做 `F.one_hot(...,E)` 得到 `[B*T,K,E]`，先对
top-k 轴 `sum(dim=1)`，再 `view(B,T,E)` 并沿时间求均值、除以 $K$。因此正式
报告的 aux-loss 结论来自 sequence-wise 公式，而不是上面的全局 top-1 proxy。
